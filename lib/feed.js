...
function Feed (drive, id, opts) {
  ...
  function open (cb) {
    ...
    self._drive._feeds.get(self.id.toString('hex'), function (_, feed) {
      if (feed) {
      ...
      self._prefix = prefix(self._pointer.toString('hex'), '')
      self._drive._bitfields.get(self.id.toString('hex'), function (_, buf) {
        self.bitfield = bitfield(buf || self.blocks || 1, BITFIELD_OPTIONS)
        self._drive.swarm.join(self.id)
        self.opened = true
        self.emit('open')
        cb()
      })
  ...
  function sync (link, cb) {
    ...
    if (link) {
      ...
      self._drive._bitfields.put(self.id.toString('hex'), self.bitfield.buffer, cb)
    }
  }
}
