...
function Feed (drive, id, opts) {
  if (!(this instanceof Feed)) return new Feed(drive, id, opts)
  if (!opts) opts = {}
  ...
  function open (cb) {
    ...
    self._drive._feeds.get(id.toString('hex'), function (_, feed) {
      if (feed) {
      ...
      self._prefix = prefix(self._pointer.toString('hex'), '')
      self._drive._bitfields.get(id.toString('hex'), function (_, buf) {
        self.opened = true
        self.emit('open')
  ...
  function sync (link, cb) {
    self.emit('update')

    if (link) {
      self._drive.db.batch([{
        type: 'put',
      ...
      self._drive._bitfields.put(id.toString('hex'), self.bitfield.buffer, cb)
    }
  }
}
...
Feed.prototype._append = function (values, cb) {
  ...
  for (var h = 0; h < values.length; h++) {
    var nodes = this._generator.next(values[i])
  ...
  this._drive.db.batch(batch, this._afterAppend)
}
...
Feed.prototype.get = function (block, cb) {
  ...
  drive._hashes.get(this._prefix + (2 * block), function (err, hash) {
    if (err) return cb(err)
    drive._blocks.get(hash.toString('hex'), cb)
  })
}
...
Feed.prototype.proof = function (block, have, cb) {
  ...
  getHashes(this._drive._hashes, this._prefix, proof, cb)
}
...
Feed.prototype.put = function (block, data, proof, cb) {
  ...
  var offset = 0
  var swap = false
  var hashes = this._drive._hashes
  ...
  function write (err) {
    if (err) return cb(err)
    self._drive._blocks.put(digest.hash.toString('hex'), data, finalize)
  }
...
Feed.prototype._putRoots = function (block, data, proof, cb) {
  ...
  this._drive._hashes.batch(batch, function (err) {
    if (err) return cb(err)
...
//   return {
//     db: db,
//     _hashes: subleveldown(db, 'hashes', {valueEncoding: 'binary'}),
//     _blocks: subleveldown(db, 'blocks', {valueEncoding: 'binary'}),
//     _bitfields: subleveldown(db, 'bitfields', {valueEncoding: 'binary'})
//   }
// }
...
// f.append('toshy')
// f.finalize(function () {
//   f.proof(5, function (err, proof) {
//     if (err) throw err
//     var f1 = Feed(drive(), f.id)
//     f.get(5, function (err, block) {
//       if (err) throw err
//       f1.put(5, block, proof, console.log)
//     })
//   })
// })
